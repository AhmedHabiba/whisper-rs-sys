...
fn main() {
    ...
    if env::var("WHISPER_DONT_GENERATE_BINDINGS").is_ok() {
        ...
        match bindings {
            ...
            // w194: upstream falls back to the checked-in bindings here with
            // only a cargo:warning. In a fork that carries its own whisper.cpp
            // that is silent breakage — the shipped bindings would describe a
            // different header than the one we just compiled. Fail loudly;
            // WHISPER_DONT_GENERATE_BINDINGS is still the deliberate opt-out.
            Err(e) => {
                panic!(
                    "Unable to generate bindings from the vendored whisper.cpp headers: {}. \
                     Set WHISPER_DONT_GENERATE_BINDINGS=1 to use src/bindings.rs on purpose.",
                    e
                );
            }
        }
...
fn get_whisper_cpp_version(whisper_root: &std::path::Path) -> std::io::Result<Option<String>> {
    ...
    // whisper.cpp <= 1.8.3 declared `project("whisper.cpp" VERSION x.y.z)`.
    // whisper.cpp >= 1.9.0 dropped that line and sets three cache variables
    // instead (WHISPER_VERSION_MAJOR/MINOR/PATCH). Accept both.
    ...
    for line in cmake_lists.lines() {
        ...
        let t = line.trim();
        for (key, slot) in [
            ("set(WHISPER_VERSION_MAJOR ", &mut major),
            ("set(WHISPER_VERSION_MINOR ", &mut minor),
            ("set(WHISPER_VERSION_PATCH ", &mut patch),
        ] {
            if let Some(rest) = t.strip_prefix(key) {
                *slot = Some(rest.trim_end_matches(')').trim().to_string());
            }
        }
    }
